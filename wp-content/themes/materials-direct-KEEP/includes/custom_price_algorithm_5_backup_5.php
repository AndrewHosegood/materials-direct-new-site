...
function add_custom_shipping_fee($cart) {
    ...
    // Loop through cart items to sum up shipping fees
    foreach ($cart->get_cart() as $cart_item) {
        if (isset($cart_item['custom_inputs']['final_shipping'])) {
            $final_shipping = floatval($cart_item['custom_inputs']['final_shipping']);
            $quantity = isset($cart_item['custom_inputs']['qty']) ? intval($cart_item['custom_inputs']['qty']) : 1;
            $total_shipping_fee += $final_shipping;
        } 
    } 

    // Add the shipping fee to the cart if greater than zero
    if ($total_shipping_fee > 0) {
        $cart->add_fee(__('Shipping', 'woocommerce'), $total_shipping_fee, true);
    }
}
// ADD ADDITIONAL FEE TO CART/CHECKOUT
...
function display_custom_inputs_on_product_page() {
    ...
    if ($part_width_mm > 0 && $part_length_mm > 0 && $quantity > 0) {
        ...
        echo '<p><strong>Shipments:</strong> ' . esc_html($final_shipping) . '</p>';
