...
function group_shipping_by_date($cart) {
    ...
    foreach ($cart->get_cart() as $cart_item) {
        if (isset($cart_item['custom_inputs']['shipments'], $cart_item['custom_inputs']['total_del_weight'], $cart_item['custom_inputs']['final_shipping'])) {
            ...
            $shipping_by_date[$date]['final_shipping'] = calculate_shipping_cost($shipping_by_date[$date]['total_del_weight']);
        }
    }
...
function calculate_shipping_cost($total_del_weight) {
...
function add_custom_price_cart_item_data_secure($cart_item_data, $product_id) {
    ...
    $final_shipping = calculate_shipping_cost($total_del_weight);
...
function add_custom_shipping_fee($cart) {
    ...
    // Add a fee for each dispatch date
    foreach ($shipping_by_date as $date => $data) {
        $final_shipping = $data['final_shipping'];
        if ($final_shipping > 0) {
            $fee_name = sprintf('Dispatch %s (%s%.2f)', $date, get_woocommerce_currency_symbol(), $final_shipping);
            $cart->add_fee($fee_name, $final_shipping, true);
        }
    }
}
// ADD ADDITIONAL FEE TO CART/CHECKOUT
...
function display_custom_inputs_on_product_page() {
    ...
    if ($part_width_mm > 0 && $part_length_mm > 0 && $quantity > 0) {
        ...
        if (!is_numeric($product_weight) || $product_weight <= 0) {
            ...
            $total_del_weight = $totalSqCm * floatval($product_weight) * $quantity * 1.03;
            /*
            if (have_rows('table_of_rates', 'option')) {

                while (have_rows('table_of_rates', 'option')) {
                    the_row();
            ...
            */
            if ($total_del_weight >= 0 && $total_del_weight < 30) {
                $final_shipping = 16.50;
            } elseif($total_del_weight >= 30 && $total_del_weight < 50){
                $final_shipping = 36.50;
            } elseif($total_del_weight >= 50){
                $final_shipping = 82.50;
            } else {
                $final_shipping = 0;
            }
